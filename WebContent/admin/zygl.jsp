...
<script type="text/javascript">
		...
		$('#admin_zygl_treegrid').treegrid({
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					return formatString('<img onclick="admin_zygl_editFun(\'{0}\');" src="{1}"/>&nbsp;<img onclick="admin_zygl_deleteFun(\'{2}\');" src="{3}"/>', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/pencil.png', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/cancel.png');
				}
			} ] ],
			toolbar : [ {
				text : '增加',
	...
	function admin_zygl_appendFun() {
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/admin/zyglAdd.jsp',
			width : 500,
			height : 200,
			modal : true,
			title : '资源添加',
	...
	function admin_zygl_editFun(id) {
		...
		var node = $('#admin_zygl_treegrid').treegrid('getSelected');
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/admin/zyglEdit.jsp',
			width : 500,
			height : 200,
			modal : true,
			title : '资源编辑',
